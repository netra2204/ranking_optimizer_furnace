...
def _converge_conversion_band(df: pd.DataFrame,
    ...
    # Loop-While body.
    # RapidMiner's Loop (While) (45) (.rmp line 6061) has NO iteration cap; it
    # only stops on `condition_satisfy == 1` (or the 86400s wall-clock guard).
    # Each pass nudges the current min/max row by +/-1 toward its limit, so the
    # worst case is bounded by the conversion span (in whole percent) across all
    # rows. We size a generous safety bound from that span instead of a fixed 50
    # so we never truncate a genuine RapidMiner convergence, while still
    # guaranteeing termination.
    span = 0.0
    if "New_Overall_conversion_limit" in work.columns:
        span = float((work["New_Overall_conversion_limit"]
                      - work["New_Overall_conversion"]).abs().sum())
    MAX_ITERS = max(200, int(np.ceil(span)) + len(work) + 10)
...
def _topN_bound_assignment(df: pd.DataFrame) -> pd.DataFrame:
    ...
    count_of_top_rows = 2
    iter_loop = 0
    # RapidMiner's top-N Loop (While) has no artificial cap; it terminates only
    # on `Conversion_Limits_given == 1` or `iter > Number_of_rows` (both handled
    # at the bottom of the loop). The previous `MAX_ITER` guard was redundant
    # (it was always larger than Number_of_rows) and is removed for fidelity.
    while True:
        iter_loop += 1
...
def _enumerate_conversion_grid(df_per_row: pd.DataFrame) -> Dict[str, float]:
    ...
    if total_combos > MAX_CONV_COMBOS:
        # RapidMiner's optimize_parameters_grid enumerates EVERY combination
        # (no cap). To stay faithful we do not truncate; we only warn so the
        # operator is visible. (Speed is deferred to the optimisation phase.)
        logger.warning("CONV GRID: %d combos exceeds soft cap %d – enumerating "
                       "all anyway to match RapidMiner (no truncation).",
    ...
    for combos_done, combo in enumerate(itertools.product(*ranges[:n])):
        # No truncation: RapidMiner evaluates the full Cartesian product.
        combo = list(combo)
