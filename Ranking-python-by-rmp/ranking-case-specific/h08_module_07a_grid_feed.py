...
try:
    import h07_module_07b_grid_conversion as gridB
...
def _enumerate_feed_grid(df: pd.DataFrame, ranges: List[List[float]]) -> Dict:
    ...
    # Iterate the Cartesian product of per-furnace value lists.
    ...
    for combo in itertools.product(*ranges):
        ...
        combo = _apply_loop49_override(list(combo))
        ...
        if compare != 0:
            ...
            # Branch (113) (.rmp line 7864): the RapidMiner condition is
            #     parse(%{sum_del_ethylene}) == parse(%{Max_Benefit})
            # i.e. commit whenever THIS combo's sum_del_ethylene equals the
            # running Max_Benefit (which Generate Macro (44) has just set to
            # sum_del_ethylene iff the combo was accepted). This is NOT the same
            # as "Max_Benefit strictly increased": because flag_benefit can fire
            # on an SPC-favourable trade where sum_del_ethylene is slightly
            # *below* the previous Max_Benefit, Generate Macro (44) can LOWER
            # Max_Benefit and Branch (113) still commits. We now replicate the
            # exact equality test rather than the strict-increase gate.
            if abs(_m("sum_del_ethylene", -1e9) - _m("Max_Benefit", -1e9)) < 1e-9:
